...
function LoanStepsCourseDetails() {
  ...
  const handleSaveChildDetails = () => {
    ...
    if (!schoolName)
      validationErrors.schoolName = "School/Institute name is required";
    if (!className)
      validationErrors.courseName = "Class/Course name is required";
    if (!annualFees)
      validationErrors.annualFees = "Total annual fees is required";
    ...
    if (result) {
      switch (result.status) {
        case "SUCCESS":
          ...
          navigate("/bank-select", { state: { data1, data3 } });
  ...
  const handleSchoolNameChange = async (
    ...
    if (value.length >= 5) {
      try {
        ...
        if (response.data.message === "Successful") {
          const options: SchoolOption[] = response.data.data.map(
            (school: any) => ({
              key: school.id,
              value: school.brandName,
              text: school.brandName,
          ...
          console.error(
            "Failed to fetch school options:",
                ...
                <>
                  ...
                  <div
                    style={{
                      ...
                      borderRadius: isChildDetailsMinimized
                        ? "12px"
                        : "12px 12px 0px 0px",
                      ...
                      >
                        ...
                        {schoolOptions.length > 0 && (
                          <Autocomplete
                            ...
                            value={
                              schoolOptions.find(
                                (option) => option.value === schoolName
                              ) || null
                            }
                            onChange={(e, newValue) => {
                              if (newValue) {
                        ...
                        <InputText
                          ...
                          value={annualFees}
                          changeHandler={(e) =>
                            handleInputChange("annualFees", e.target.value)
                          }
                        />
                        {errors.annualFees && (
                          <span style={{ color: "red" }}>
